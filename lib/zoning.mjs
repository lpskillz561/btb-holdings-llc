...
/**
 * The guard on the swap: two records are the same parcel only if their street
 * numbers agree.
 *
 * Deliberately weak-but-safe. It does not try to reconcile street names, which
 * are abbreviated differently by the two publishers ("N FORT WILDERNESS TRL"
 * vs "FORT WILDERNESS TRAIL") and would reject good matches. A house number is
 * the cheapest field that a colliding id will almost never agree on.
 *
 * UNKNOWN IS NOT AGREEMENT. If either side has no number - vacant land often
 * has none - this returns false and the row is skipped. A zoning code we cannot
 * prove belongs to the parcel is worse than no zoning code, because the reader
 * cannot tell the difference.
 */
export function addressesAgree(ourAddr, theirAddr) {
